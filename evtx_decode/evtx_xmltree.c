...
static void xml_dump_attrs(XML_ELEMENT *e)
{
    for (uint16_t i = 0; i < e->attr_count; i++) {
        printf(" %s=\"%s\"", e->attrs[i].name, e->attrs[i].value);
    }
}
...
int xml_add_attribute(XML_ELEMENT *elem, const char *name, const char *value)
{
    if (!elem || !name || !value) return -1;

    // prevent overflow in attr_count
    if (elem->attr_count == UINT16_MAX) return -1;

    uint16_t new_count = (uint16_t)(elem->attr_count + 1);

    // realloc attribute array
    XML_ATTRIBUTE *new_attrs = (XML_ATTRIBUTE *)realloc(
        elem->attrs, (size_t)new_count * sizeof(XML_ATTRIBUTE)
    );
    ...
    // initialize new slot
    XML_ATTRIBUTE *a = &elem->attrs[elem->attr_count];
    a->name  = NULL;
    a->value = NULL;
